...
void loop() {
  ...
  // Hardware inputs are optional - disable to prevent interference with UI button presses
  // The hardware inputs (openInput/closeInput) may read HIGH due to pull-ups,
  // which causes the breaker to immediately reopen after closing via UI button
  // Uncomment and add debouncing if you need to use external hardware switches:
  /*
  if (!bluetoothConnected) {
...
static void close_btn_cb(lv_event_t *e) {
  ...
  // Allow closing if switch is UP and breaker is OPEN
  // This is the normal operation - switch must be up to close the breaker
  if (switchToggled && breakerstate) {
    ...
    Serial.println("Close button ignored - switch must be UP to close breaker");
    Serial.print("Current state - switchToggled=");
    Serial.print(switchToggled ? "UP" : "DOWN");
    Serial.print(", breakerstate=");
    Serial.println(breakerstate ? "OPEN" : "CLOSED");
  } else if (!breakerstate) {
    Serial.println("Close button ignored - breaker is already CLOSED");
  } else {
...
static void settings_btn_cb(lv_event_t *e) {
  ...
  if (!settings_modal) {
    ...
    // Align modal to center but offset down slightly to account for 70px top bar (matching BS69)
    lv_obj_align(settings_modal, LV_ALIGN_CENTER, 0, 25);
  ...
  if (settings_modal) {
    // Show settings modal by centering it, but offset down slightly to avoid top bar (matching BS69)
    lv_obj_align(settings_modal, LV_ALIGN_CENTER, 0, 25);
  }
